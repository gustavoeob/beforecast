  ...
  const [t] = useTranslation("global")
    ...
    return (
      <section className="current-weather-box">
        <h2 className={`color-${data.weather[0].icon}`}>{t("title.weather")}</h2>
          ...
          <div className="widget-middle-box">
            <div className="weather-time">
              <div className="min-max-box">
                <span className="">
                  min {Math.floor(data.main.temp_min)}{unit === "metric" ? "°C" : "°F"}{" "}
                </span>
                <br />
                <span className="">max {Math.ceil(data.main.temp_max)}{unit === "metric" ? "°C" : "°F"} </span>
              </div>
            </div>
            <div className="temperature-box">
              <span className="temperature">{Math.round(data.main.temp)}</span>
              <span className="temperature-scale">{unit === "metric" ? "°C" : "°F"}</span>
            </div>
          </div>
          <div className="widget-bottom-box">
            <div className="weather-details">
              ...
              <div className="params">
                ...
                <p className="param-value">{data.wind.speed} {unit === "metric" ? "m/s" : "m/h"}</p>
              </div>
              <div className="params">
